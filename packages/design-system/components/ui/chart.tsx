...
// biome-ignore lint/performance/noNamespaceImport: Recharts groups chart primitives and helper types under one package namespace.
...
type ChartTooltipFormatter = RechartsPrimitive.DefaultTooltipContentProps<
  TooltipValueType,
  TooltipNameType
>["formatter"];
      ...
      <div className="grid gap-1.5">
        {payload
          ...
          .map((item, itemIndex) => {
            ...
            const itemKey = getPayloadItemKey(item, configKey);
...
function getPayloadItemKey(item: TooltipPayloadItem, key: string) {
  return `${key}-${item.color ?? ""}-${item.value ?? ""}`;
}
